    ...
    for pathStep in pathList:
        ...
        for line in windowLines:
            ...
            if winTarget in guideCountDict.keys():
                ...
                guideCountDict[winTarget]['PAM'].append(winPAM)
                ...
                winDict = {'accW':[winAcc], 'start':[winStart], 'target':[winTarget], 'spacer':[winSpacer], 'strand':[winStrand], 'PAM': [winPAM],'GC':[winGC], 'A':[winA], 'accM':[curAcc]}
                for metaKey in curMeta.keys():
...
def sumCountDict(guideCountDict, accList):
    ...
    guideChunkSumDict = {'accession':[], 'start':[], 'target':[], 'total_hit':[], 'strains_hit':[], 'subtypes_hit':[], 'spacer':[], 'strand':[], 'PAM':[],'GC_content':[], "A_content":[]}
    for clade in topCladesDictSorted.keys():
    ...
    for target in guideCountDict.keys():
        ...
        guideChunkSumDict['PAM'].append(targetDict['PAM'][firstIndex])
