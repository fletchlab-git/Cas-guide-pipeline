...
def main():
    ...
    for record in SeqIO.parse(args.input, "fasta"):
        sanitized_id = record.id.replace("/", "_")
        if not os.path.exists(f"{args.out}/{sanitized_id}"):
            os.makedirs(f"{args.out}/{sanitized_id}")
        with open(f"{args.out}/{sanitized_id}/{sanitized_id}.fa", "w") as handle:
            handle.write(f">{record.description}\n{record.seq}\n")
        list_of_genomes.append(sanitized_id)
        genome_count += 1
